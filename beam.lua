...
function generate_beam(model, initial, K, max_sent_l, source, gold)
    ...
    if model_opt.brnn == 1 then
        ...
        if model_opt.init_dec == 1 then
            for L = 1, model_opt.num_layers do
                ...
                rnn_state_dec[L * 2 + model_opt.input_feed]:add(rnn_state_enc[L * 2]:expand(K, model_opt.rnn_size))
            end
        end
    end
    ...
    if opt.score_gold == 1 then
        ...
        for t = 2, target_l do
            ...
            saved_soft_attn[saved_decoder_position]:copy(decoder_softmax.output)
...
function main()
    ...
    for line in file:lines() do
        ...
        pred_score, gold_score = generate_beam(model, state, opt.beam, MAX_SENT_L, source, target)
